...
from BackGroundChanging.util.dataloader import (
...
class Trainer:
    def __init__(self, args, save_path):
        ...
        self.train_transform = get_train_augmentation(
            img_size=args.img_size, ver=args.aug_ver
        )
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if val_loss < min_loss:
                ...
                min_loss = val_loss
                torch.save(
                    self.model.state_dict(), os.path.join(save_path, "best_model.pth")
                )
        ...
        for dataset in datasets:
            ...
            test_loss, test_mae, test_maxf, test_avgf, test_s_m = self.test(
                args, os.path.join(save_path)
            )
    ...
    def test(self, args, save_path):
        ...
        with torch.no_grad():
            for i, (images, masks, original_size, image_name) in enumerate(
                tqdm(test_loader)
            ):
                images = torch.tensor(images, device=self.device, dtype=torch.float32)
                ...
                for i in range(images.size(0)):
                    ...
                    # Metric
                    mae, max_f, avg_f, s_score = Eval_tool.cal_total_metrics(
                        output, mask
                    )
...
class Tester:
    def __init__(self, args, save_path):
        ...
        if args.save_map is not None:
            os.makedirs(
                os.path.join("mask", "exp" + str(self.args.exp_num), self.args.dataset),
                exist_ok=True,
            )
    ...
    def test(self):
        ...
        print(
            f"Test Loss:{test_loss:.4f} | MAX_F:{test_maxf:.4f} | MAE:{test_mae:.4f} "
