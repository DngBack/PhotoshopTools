...
class DatasetGenerate(Dataset):
    def __init__(
        ...
        (
            ...
            self.images, self.gts, self.edges, test_size=0.05, random_state=seed
        )
        if phase == "train":
...
class Test_DatasetGenerate(Dataset):
    def __init__(self, img_folder, gt_folder=None, transform=None):
        self.images = sorted(glob.glob(img_folder + "/*"))
        self.gts = (
            sorted(glob.glob(gt_folder + "/*")) if gt_folder is not None else None
        )
        self.transform = transform
...
def get_loader(
    ...
    if phase == "test":
        ...
        dataset = DatasetGenerate(
            img_folder, gt_folder, edge_folder, phase, transform, seed
        )
        data_loader = DataLoader(
            dataset,
